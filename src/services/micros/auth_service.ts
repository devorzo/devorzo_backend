...
const authService = (app: express.Application) => {
  ...
  router.get("/api/:version/auth-service", (req, res) => {
    ...
    if (version == "v1") {
      ...
      res
        .status(400)
        .send(responseMessageCreator("Invalid API version provided!", 0));
    }
  });
  ...
  router.post("/login", auth_IS_LOGGED_OUT, auth_controller.loginController);

  router.post("/logout", auth_IS_LOGGED_IN, auth_controller.logoutController);

  router.post(
    "/api/:version/verifyEmail",
    auth_IS_LOGGED_IN,
    auth_controller.VerifyEmail
  );

  router.get("/api/:version/reset");
  router.post("/oauth", auth_IS_LOGGED_OUT, auth_controller.handleOauth);
  app.use(router);
};
