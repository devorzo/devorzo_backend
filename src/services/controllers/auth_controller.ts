...
export const registerController = (req: Request, res: Response) => {
  ...
  if (
    ...
    res
      .status(400)
      .send(responseMessageCreator(`Missing required arguments: ${m}`, 0));
  }
              ...
              .then((token) => {
                ...
                follower
                  ...
                  .then(() => {
                    ...
                    res.send(
                      responseMessageCreator({user: user.toJSON(), token}, 1),
                    );
                  })
                  .catch((e) => {
                    res.setHeader('x-auth', token);
  ...
  User.findOne({
    ...
    if (doc) {
      if (doc.email_verified == 1) {
        res
          .status(400)
          .send(responseMessageCreator('Email is already verified', 0));
      } else {
...
export const handleOauth = async (req: Request, res: Response) => {
  ...
  const providerIdKey = provider === 'github' ? 'github_id' : 'twitter_id';
  const exisitingUser = await User.findOne({[providerIdKey]: profile.id});

  console.log('existint user is ', exisitingUser, providerIdKey, profile.id);
  if (exisitingUser) {
    // login case
    if (body.clear_tokens) {
      await exisitingUser.removeAllAuthToken();
    }
    const token = await exisitingUser.generateAuthToken();
    if (req.body.cache) {
      if (req.session && body.session_persistance) {
        req.session.xAuth = token;
        req.session.uid = exisitingUser._id;
        req.session.name = exisitingUser.email;
      }
      res.setHeader('x-auth', token);
    }
    res.send(responseMessageCreator({user: exisitingUser.toJSON(), token}, 1));
  } else {
    const isUnique = await User.isUsernameUnique(profile.login);
    const _newUser = new User({
      email: profile.email,
      details: {
        fullname: profile.name,
        username: isUnique ? profile.login : profile.email,
      },
      password: 'oauth_pass',
      user_id: `user.${uuid()}`,
      [providerIdKey]: profile.id,
    });
    const data = await User.accountExists(body.email);
    if (!data) {
      if (body.confpassword == body.password) {
        const createdUser = await _newUser.save();
        const token = await createdUser.generateAuthToken();
        logger({user: _newUser});
        let follower = new Follower({
          user_id: _newUser.user_id,
        });
        await follower.save();
        res.setHeader('x-auth', token);
        return res.send(
          responseMessageCreator({user: createdUser.toJSON(), token}, 1),
        );
      } else {
        let message = 'Passwords do not match!';
        res.status(400).send(responseMessageCreator(message, 0));
